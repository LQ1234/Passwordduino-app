...
class EntryTableViewCell: UITableViewCell {
    ...
    @IBAction func executeButtonPressed(_ sender: UIButton) {
        ...
        if(entry.entryType == .password){
            Bytecode.from(name:entry.name, passwordInfo: (entries:viewController.entries, controller: viewController)) { (res, error) in
                guard let res=res else{
                    infoPrompt(title:"Ducky Script Error",message:error,controller:self.viewController){_ in}
                    return;
                }
                print("parsed bytecode");
                bluetoothWrapper.selectedPasswordduino?.sendEncrypted(data: res, callback: { (error) in
                    if let error=error{
                        infoPrompt(title:"Bluetooth Error",message:error,controller:self.viewController){_ in}
                    }
                })
            }
        }else{
            Bytecode.from(ducky: entry.contents, passwordInfo: (entries:viewController.entries, controller: viewController)) { (res, error) in
                guard let res=res else{
                    infoPrompt(title:"Ducky Script Error",message:error,controller:self.viewController){_ in}
                    return;
                }
